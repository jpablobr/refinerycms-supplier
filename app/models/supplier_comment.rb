class SupplierComment < ActiveRecord::Base
  ...
  belongs_to :post, :class_name => 'SupplierPost', :foreign_key => 'supplier_post_id'
  ...
  def self.toggle!
    ...
    RefinerySetting.set(:comments_allowed, {:value => !currently, :scoping => 'supplier'})
  end

  before_create do |comment|
    unless SupplierComment::Moderation.enabled?
  ...
  module Moderation
    class << self
      ...
      def toggle!
        new_value = {
          :value => !SupplierComment::Moderation.enabled?,
          :scoping => 'supplier',
          :restricted => false
        }
